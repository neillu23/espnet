...
class Speech2Text:
    ...
    def __init__(
        ...
        joint_model.eval()
        self.num_spk = joint_model.num_spk
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if "speech_ref1" in kwargs:
            speech_ref = [
                torch.as_tensor(kwargs["speech_ref{}".format(spk + 1)]).to(
        ...
        if self.joint_model.cal_enh_loss and speech_ref is not None:
            # nspk,T
            ref = torch.stack([sr.squeeze() for sr in speech_ref], dim=0).cpu().numpy()
            inf = torch.stack([sp.squeeze() for sp in speech_pre], dim=1).cpu().numpy()
