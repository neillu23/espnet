...
def cat(tensors, dim=0, out=None):
    assert all(map(lambda x: type(x) == type(tensors[0]), tensors[1:]))
    if isinstance(tensors[0], ComplexTensor):
        return FC.cat(tensors, dim=dim, out=out)
    else:
...
def einsum(equation, *operands):
    ...
    if len(operands) == 1 and isinstance(operands[0], (tuple, list)):
        operands = operands[0]
    func_module = FC if isinstance(operands[0], ComplexTensor) else torch
    if isinstance(operands[0], ComplexTensor):
        test = lambda x: isinstance(x, ComplexTensor)
    elif is_torch_complex_tensor(operands[0]):
        test = lambda x: is_torch_complex_tensor(x)
    else:
        test = lambda x: True
    if all(map(test, operands[1:])):
        # all operands have the same real/complex type
        return func_module.einsum(equation, *operands)
    elif len(operands) != 2:
        raise ValueError("0 or More than 2 mixed-type operands are not supported.")
    a, b = operands
...
def stack(tensors, dim=0, out=None):
    assert all(map(lambda x: type(x) == type(tensors[0]), tensors[1:]))
    if isinstance(tensors[0], ComplexTensor):
        return FC.stack(tensors, dim=dim, out=out)
    else:
