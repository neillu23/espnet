...
class ESPnetEnhancementModel(ESPnetEnhancementModelSupervised, AbsESPnetModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if getattr(self, "supervised_loss_wrapper", None) is None:
            raise ValueError("Supervised loss wrapper is required.")
        if getattr(self, "unsupervised_loss_wrapper", None) is None:
            raise ValueError("Unsupervised loss wrapper is required.")
    ...
    def forward(
        ...
        **kwargs,
        ...
        if utt_ids[0].endswith("_supervised"):  # e.g. 011_012-011c0213_0.85538_012o030a_-0.85538_supervised
            loss_wrapper_type = "pit"
        else :                 # e.g. 911_6880-911-128684-0038_6880-216547-0054_unsupervised
    ...
    def _generate_mixture_of_mixtures(
        ...
        if len(mixture_of_batch_idx) > 0:
            ...
            for item in mixture_of_batch_idx:
                ...
                # speech_lengths_unsupervised.append(
                #     torch.stack(
                #         [speech_lengths[item[0]], speech_lengths[item[1]]],
                #         axis=0,
                #     )   # (num_mixture_of_mixtures)
                # )

                # Supervised reference
                # speech_ref_supervised = None
                # speech_ref_supervised.append(
                #     torch.stack(
                #         torch.concatenate(
                #             [speech_ref[item[0]], speech_ref[item[1]]],
                #             axis=0,
                #         )  # (num_mixture_of_mixtures * num_spkrs, samples, ...)
                #     )
                # speech_lengths_supervised.append(
                #     torch.contenate(
                #         [speech_lengths[item[0]], speech_lengths[item[1]]],
                #         axis=0,
                #     )
            ...
            # speech_ref_supervised = torch.stack(speech_ref_supervised, axis=0)  # (batch, num_mixture_of_mixtures * num_spkrs, samples, ...)
            # speech_lengths_unsupervised = torch.stack(speech_lengths_unsupervised, axis=0)  # (batch, )
        else:
