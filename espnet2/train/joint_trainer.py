...
class JointTrainer(Trainer):
    ...
    def validate_one_epoch_spk(
        ...
        if distributed:
            ...
            lengths_all = [to_device(torch.zeros(1, dtype=torch.int32), "cuda") for _ in range(torch.distributed.get_world_size())]

            torch.distributed.barrier()
            torch.distributed.all_gather(lengths_all, length)

            lengths_all = [length.item() for length in lengths_all]  # Extract values
            max_length = max(lengths_all)

            # Pad scores and labels to the maximum length
            padded_scores = torch.zeros(max_length, dtype=scores.dtype).to(scores.device)
            padded_scores[:scores.size(0)] = scores
            padded_labels = torch.zeros(max_length, dtype=labels.dtype).to(labels.device)
            padded_labels[:labels.size(0)] = labels

            scores_all = [torch.zeros(max_length, dtype=torch.float32).to("cuda") for _ in lengths_all]
            labels_all = [torch.zeros(max_length, dtype=torch.int32).to("cuda") for _ in lengths_all]

            torch.distributed.barrier()

            torch.distributed.all_gather(scores_all, padded_scores)
            torch.distributed.all_gather(labels_all, padded_labels)

            # Concatenate and trim the padded tensors to their original lengths
            scores = torch.cat(scores_all)[:sum(lengths_all)]
            labels = torch.cat(labels_all)[:sum(lengths_all)]

            # Ensure all processes are synchronized after the gathering
            torch.distributed.barrier()
