...
asr_config=conf/tuning/train_asr_xlsr_10min_ca_tcac.yaml
...
train_set=train_${duration}${suffix}_asr_lid
train_dev=dev_${duration}${suffix}
# test_set="${train_dev} test_${duration}${suffix}"
test_set="test_${duration}${suffix}_lid"
...
./asr.sh \
    ...
    --test_sets "${test_set}" \
    --asr_tag "${asr_tag}" \
    --asr_stats_dir exp_asr_lid_joint/asr_stats_multilingual_asr_lid_${duration}/ \
    --local_score_opts "true ${only_lid} normal" 
