...
def copy_model_parameters(pretrained_model, film_model):
    ...
    update_state_dict = {name: param for name, param in pretrained_state_dict.items() if (name in film_state_dict) and ("frontend.upstream" not in name) and (param.shape == film_state_dict[name].shape)}

    film_update_state_dict = {}
    for name, param in film_state_dict.items():
        if "frontend.upstream" in name:
            continue
