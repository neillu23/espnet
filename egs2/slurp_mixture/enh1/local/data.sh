...
if [ -z "${SLURP_MIX_MC}" ]; then
    log "Fill the value of 'SLURP_MIX_MC' of db.sh"
    exit 1
fi


if [ -z "${LIBRITRANS_MIX_MC}" ]; then
    log "Fill the value of 'LIBRITRANS_MIX_MC' of db.sh"
    exit 1
fi
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    python3 local/prepare_mixture_data.py ${SLURP} ${SLURP_MIX_MC} ${LIBRITRANS_MIX_MC}
    for x in test devel train; do
        # Write wav.scp base on spk1.scp
