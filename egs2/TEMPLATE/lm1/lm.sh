...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ] && ! [[ " ${skip_stages} " =~ [[:space:]]6[[:space:]] ]]; then
    log "Stage 6: LM collect stats: train_set=${data_feats}/${train_set}/lm_train.txt, dev_set=${lm_dev_text}"
    ...
    _nj=$(min "${nj}" "$(<${data_feats}/${train_set}/lm_train.txt wc -l)" "$(<${lm_dev_text} wc -l)")

    key_file="${data_feats}/${train_set}/lm_train.txt"
    split_scps=""
    for n in $(seq ${_nj}); do
    ...
    ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
        ${python} -m espnet2.bin.lm_train \
            ...
            --cleaner "${cleaner}" \
            --g2p "${g2p}" \
            --train_data_path_and_name_and_type "${data_feats}/${train_set}/lm_train.txt,text,text" \
...
if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ] && ! [[ " ${skip_stages} " =~ [[:space:]]7[[:space:]] ]]; then
    log "Stage 7: LM Training: train_set=${data_feats}/${train_set}/lm_train.txt, dev_set=${lm_dev_text}"
    ...
    if [ "${num_splits_lm}" -gt 1 ]; then
        ...
        if [ ! -f "${_split_dir}/.done" ]; then
            ...
            ${python} -m espnet2.bin.split_scps \
              --scps "${data_feats}/${train_set}/lm_train.txt" "${lm_stats_dir}/train/text_shape.${token_type}" \
        ...
        _opts+="--train_data_path_and_name_and_type ${data_feats}/${train_set}/lm_train.txt,text,text "
