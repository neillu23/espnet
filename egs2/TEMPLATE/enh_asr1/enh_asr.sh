...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.enh_asr_train \
                ...
                --train_data_path_and_name_and_type "${_asr_train_dir}/spk1.scp,speech_ref1,${_type}" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/${_scp},speech,${_type}" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/text,text,text" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/spk1.scp,speech_ref1,${_type}" \
                --train_shape_file "${_logdir}/train.JOB.scp" \
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
            ...
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/spk1.scp,speech_ref1,${_type} "
            _opts+="--train_shape_file ${asr_stats_dir}/train/speech_shape "
            _opts+="--train_shape_file ${asr_stats_dir}/train/text_shape.${token_type} "
            _opts+="--train_shape_file ${asr_stats_dir}/train/speech_ref1_shape "
        fi
        ...
        ${python} -m espnet2.bin.launch \
            ...
            ${python} -m espnet2.bin.enh_asr_train \
                ...
                --valid_shape_file "${asr_stats_dir}/valid/speech_ref1_shape" \
                --resume true \
    ...
    if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
        ...
        for dset in ${test_sets}; do
            ...
            if "${use_k2}"; then
              ...
              asr_inference_tool="espnet2.bin.enh_asr_inference"
            fi
