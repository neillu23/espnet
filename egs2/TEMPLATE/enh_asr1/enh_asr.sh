...
dumpdir=dump_asr     # Directory to dump features.
...
# Evaluation related
scoring_protocol="PESQ ESTOI STOI SDR SAR SIR SI_SNR"
ref_channel=0
...
if [[ "$valid_set" == *_multi_* ]]; then
    joint_exp="${joint_exp}_multiValid"
fi
if [ -z "${lm_exp}" ]; then
    ...
    if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
        ...
        if [ -n "${train_aux_set}" ]; then
            ...
            if ${use_signal_ref}; then
                ...
                if $use_signal_noise_ref; then
                    # references for denoising ("noise1 noise2 ... niose${noise_type_num} ")
                    _files+=$(for n in $(seq $noise_type_num); do echo -n "noise${n}.scp "; done)
                fi
                if $use_signal_dereverb_ref; then
                    # references for dereverberation
                    _files+=$(for n in $(seq $dereverb_ref_num); do echo -n "dereverb${n}.scp "; done)
                fi
            fi
            ./utils/combine_data.sh --skip-fix true --extra-files "$_files utt2category utt2num_samples" \
...
if ! "${skip_eval}"; then
    ...
    if [ ${stage} -le 13 ] && [ ${stop_stage} -ge 13 ]; then
        log "Stage 13: Enh inference and scoring: training_dir=${joint_exp}"
        ...
        for dset in ${test_sets}; do
            ...
            log "Inference started... log: '${_logdir}/joint_inference.*.log'"
            # shellcheck disable=SC2086
            ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/joint_inference.JOB.log \
                ${python} -m espnet2.bin.enh_asr_inf_score \
                    --ngpu "${_ngpu}" \
                    --fs "${fs}" \
                    ${_decode_data_param} \
                    --key_file "${_logdir}"/keys.JOB.scp \
            ...
            for spk in $(seq "${spk_num}"); do
                for protocol in ${scoring_protocol}; do
                    ...
                    done | LC_ALL=C sort -k1 > "${_dir}/scoring/${protocol}_spk${spk}"
                done
            done
            ...
            for protocol in ${scoring_protocol}; do
                # shellcheck disable=SC2046
                paste $(for j in $(seq ${spk_num}); do echo "${_dir}/scoring/${protocol}_spk${j}" ; done)  |
                awk 'BEGIN{sum=0}
                    {n=0;score=0;for (i=2; i<=NF; i+=2){n+=1;score+=$i}; sum+=score/n}
                    END{print sum/NR}' > "${_dir}/scoring/result_${protocol,,}.txt"
            done
        done
        ../../TEMPLATE/enh1/scripts/utils/show_enh_score.sh ${joint_exp} > "${joint_exp}/RESULTS_ENH.TXT"
    fi
else
