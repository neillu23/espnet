...
Usage:
    $0 [--fs <fs>] [--ref_channel <ref_channel>] [--nostrict <any_value>] [--out <out_file>] <ref.scp> <inf.scp>
or
...
if ! command -v "PESQ" &> /dev/null; then
    ...
    log "cd ../../../tools; installers/install_pesq.sh"
    exit 1
fi
...
if [[ "${ref##*.}" != "${inf##*.}" ]]; then
    ...
    while read -r uttid ref_path inf_path; do
        ...
        (
            cd "$tmpdir"
            pesq_score=$(PESQ +${fs} "$ref_wav" "$inf_wav" | tail -n 1 | grep -Po '(?<=PESQ_MOS = )[.0-9]+') || (
                set +e
                set -x
                info=$(PESQ +${fs} "$ref_wav" "$inf_wav" | tail -n 1)
                set +x
                >&2 echo $info
            )
            if [ -n "$out" ] && [ "$out" != "-" ]; then
    ...
    (
        cd "$tmpdir"
        pesq_score=$(PESQ +${fs} "$ref_wav" "$inf_wav" | tail -n 1 | grep -Po '(?<=PESQ_MOS = )[.0-9]+') || (
            echo -e "\nPESQ +${fs} \"$ref_wav\" \"$inf_wav\" | tail -n 1"
            PESQ +${fs} "$ref_wav" "$inf_wav" | tail -n 1
        )
        echo $pesq_score
    )
